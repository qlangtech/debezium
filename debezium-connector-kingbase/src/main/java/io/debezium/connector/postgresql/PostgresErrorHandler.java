...
public class PostgresErrorHandler extends ErrorHandler {
    ...
    protected boolean isRetriable(Throwable throwable) {
        if (isRetriableKSQLException(throwable)) {
            return true;
        }
        else if (throwable instanceof DebeziumException) {
            return isRetriableKSQLException(throwable.getCause());
        }
        return false;
    }

    public boolean isRetriableKSQLException(Throwable throwable) {
        if (throwable != null && throwable instanceof KSQLException && throwable.getMessage() != null) {
