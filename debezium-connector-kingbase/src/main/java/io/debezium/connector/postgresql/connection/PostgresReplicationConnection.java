...
public class PostgresReplicationConnection extends JdbcConnection implements ReplicationConnection {
    ...
    private PostgresReplicationConnection(PostgresConnectorConfig config,
        ...
        super(addDefaultSettings(config.getJdbcConfig()), PostgresConnection.FACTORY, null, () -> PostgresReplicationConnection.class.getClassLoader(), "\"", "\"");
    ...
    protected void initPublication() {
        ...
        if (LogicalDecoder.DECODERBUFS.equals(plugin)) {
...
//                    plugin.getPostgresPluginName());
    ...
    private ReplicationStream createReplicationStream(final Lsn startLsn, WalPositionLocator walPosition) throws SQLException, InterruptedException {
        ...
        try {
            try {
                s = startKBReplicationStream(startLsn,
                        plugin.forceRds()
                ...
                s = startKBReplicationStream(startLsn, plugin.forceRds() ? messageDecoder::optionsWithoutMetadata : messageDecoder::optionsWithMetadata);
                messageDecoder.setContainsMetadata(plugin.forceRds() ? false : true);
            }
        } catch (KSQLException e) {
    ...
    private KBReplicationStream startKBReplicationStream(final Lsn lsn,
